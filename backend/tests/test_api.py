...
class AuthenticationTestCase(TestCase):
    ...
    def test_signup(self):
        ...
        response = self.client.post(
            "/signup", json.dumps(self.test_user), content_type="application/json"
        )
    ...
    def test_login_nonexistent_user(self):
        ...
        response = self.client.post(
            "/login",
            json.dumps({"username": "doesnotexist", "password": "anypassword"}),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.json()["error"], "Invalid credentials")
...
class FieldManagementTestCase(TestCase):
    ...
    def test_delete_field(self):
        ...
        response = self.client.delete(f"/field/data/{field.id}", **self.auth_header)
...
class ValidationTestCase(TestCase):
    ...
    def test_missing_required_fields(self):
        ...
        incomplete_field = {"name": "Incomplete Field"}
