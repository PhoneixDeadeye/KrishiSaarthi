...
class SignupValidationTestCase(TestCase):
    ...
    def test_signup_missing_password(self):
        response = self.client.post(
            "/signup", {"username": "testuser", "email": "test@test.com"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class LoginTimingTestCase(TestCase):
    ...
    def test_login_nonexistent_user(self):
        response = self.client.post(
            "/login", {"username": "nobody", "password": "Whatever123!"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
...
class PasswordResetRequestTestCase(TestCase):
    ...
    def test_password_reset_request_valid_email(self):
        ...
        response = self.client.post(
            "/password-reset", {"email": "reset@test.com"}, format="json"
        )
    ...
    def test_password_reset_request_invalid_email(self):
        ...
        response = self.client.post(
            "/password-reset", {"email": "nonexistent@test.com"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
