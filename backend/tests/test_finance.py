...
class FinanceAPITestCase(TestCase):
    ...
    def test_price_forecast_success(self):
        ...
        response = self.client.get(
            "/finance/price-forecast?crop=Rice&days=30", **self.auth_headers
        )
    ...
    def test_price_forecast_different_crops(self):
        ...
        for crop in crops:
            response = self.client.get(
                f"/finance/price-forecast?crop={crop}", **self.auth_headers
            )
            self.assertEqual(response.status_code, 200)
    ...
    def test_price_forecast_unknown_crop(self):
        ...
        response = self.client.get(
            "/finance/price-forecast?crop=Mango", **self.auth_headers
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_schemes_list_success(self):
        ...
        response = self.client.get("/finance/schemes?state=Punjab", **self.auth_headers)
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_schemes_filter_by_type(self):
        ...
        for scheme_type in ["subsidy", "loan", "insurance"]:
            response = self.client.get(
                f"/finance/schemes?type={scheme_type}", **self.auth_headers
            )
            self.assertEqual(response.status_code, 200)
    ...
    def test_insurance_list_empty(self):
        ...
        response = self.client.get("/finance/insurance", **self.auth_headers)
        self.assertEqual(response.status_code, 200)
        data = response.json()
...
class MarketPricesTestCase(TestCase):
    ...
    def test_market_prices_success(self):
        ...
        response = self.client.get(
            "/finance/market-prices?crop=Rice&state=Punjab", **self.auth_headers
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_market_prices_unknown_crop(self):
        ...
        response = self.client.get(
            "/finance/market-prices?crop=Mango", **self.auth_headers
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
...
class PlanningAPITestCase(TestCase):
    ...
    def test_rotation_planner_no_field(self):
        ...
        response = self.client.get(
            "/planning/rotation?field_id=99999", **self.auth_headers
        )
        self.assertEqual(response.status_code, 404)

    def test_inventory_list(self):
        """Test inventory list endpoint"""
        response = self.client.get("/planning/inventory", **self.auth_headers)
        self.assertEqual(response.status_code, 200)

    def test_labor_list(self):
        """Test labor list endpoint"""
        response = self.client.get("/planning/labor", **self.auth_headers)
        self.assertEqual(response.status_code, 200)

    def test_equipment_list(self):
        """Test equipment list endpoint"""
        response = self.client.get("/planning/equipment", **self.auth_headers)
        self.assertEqual(response.status_code, 200)
