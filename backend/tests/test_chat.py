...
class ChatSessionModelTestCase(TestCase):
    ...
    def test_delete_session_cascades(self):
        ...
        session.delete()
        self.assertEqual(
            ChatMessage.objects.filter(session__session_id="test-session-003").count(),
            0,
        )
...
class ChatIDORPreventionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        ChatMessage.objects.create(
            session=self.session_a, role="user", text="Secret question"
        )
        ChatMessage.objects.create(
            session=self.session_a, role="model", text="Secret answer"
        )
