...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not settings.ALLOWED_HOSTS or settings.ALLOWED_HOSTS == ["*"]:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'   ✓ {", ".join(settings.ALLOWED_HOSTS)}')
            )
        ...
        if "sqlite" in db_engine and not settings.DEBUG:
            ...
            self.stdout.write(
                self.style.WARNING("   ⚠ SQLite (consider PostgreSQL for production)")
            )
        else:
        ...
        if not cors_origins:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"   ✓ {len(cors_origins)} origins allowed")
            )
        ...
        if "console" in email_backend and not settings.DEBUG:
            ...
            self.stdout.write(
                self.style.WARNING(
                    "   ⚠ Console backend (configure SMTP for production)"
                )
            )
        else:
