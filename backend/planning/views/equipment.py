        ...
        return Response(
            {
                ...
                "summary": {
                    ...
                    "available": equipment_list.filter(
                        status=EquipmentStatus.AVAILABLE
                    ).count(),
                    "in_use": equipment_list.filter(
...
class EquipmentBookingView(APIView):
    ...
    def get(self, request, equipment_id=None):
        ...
        if equipment_id:
            ...
            bookings = EquipmentBooking.objects.filter(
                equipment__user=request.user
            ).select_related("equipment", "field")
    ...
    def post(self, request, equipment_id=None):
        ...
        if equipment_id:
            ...
            equipment = get_object_or_404(
                Equipment, pk=data.get("equipment"), user=request.user
            )
    ...
    def put(self, request, pk=None):
        ...
        booking = get_object_or_404(
            EquipmentBooking, pk=pk, equipment__user=request.user
        )
        serializer = EquipmentBookingSerializer(
