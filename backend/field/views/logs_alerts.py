...
class FieldLogView(APIView):
            ...
            if activity in ACTIVITY_ALERT_DAYS:
                ...
                if days > 0 and message:
                    ...
                    FieldAlert.objects.create(
                        user=request.user, log=log, date=alert_date, message=message
                    )
    ...
    def delete(self, request, pk=None):
        ...
        if not pk:
            return Response(
                {"error": "Log ID is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
...
class FieldAlertView(APIView):
    ...
    def patch(self, request, pk=None):
        ...
        if not pk:
            return Response(
                {"error": "Alert ID is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
            ...
            alert.save()
            return Response(
                FieldAlertSerializer(alert, context={"request": request}).data
            )
