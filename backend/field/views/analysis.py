...
class PestReport(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            uploaded = Pest.objects.create(user=request.user, image=image_file)
...
class CarbonCredit(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {"error": f"Area calculation failed"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        ...
        result = calculate_carbon_metrics(
            area_hectare=area, ndwi_series=ndwi_series, ndwi_dates=ndwi_dates
        )
        if ee_warning:
...
class HealthScore(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            latest_pest = (
                Pest.objects.filter(user=request.user).order_by("-uploaded_at").first()
            )
            ...
            result = get_health_score(
                image_path=image_path, ndvi_latest=ndvi_latest, sequence=data
            )

            logger.info(
                "Health score calculated for user %s: %s",
