...
class IrrigationScheduleView(APIView):
    ...
    def get(self, request):
        ...
        if (
            not field.polygon
            or not isinstance(field.polygon, dict)
            or "coordinates" not in field.polygon
            or not field.polygon["coordinates"]
        ):
            return Response(
                {
        ...
        recent_logs = IrrigationLog.objects.filter(
            field=field, date__gte=timezone.now().date() - timedelta(days=7)
        ).order_by("-date")
        ...
        for i in range(7):
            ...
            recommendation = self._get_recommendation(
                ...
                days_since_last_irrigation=self._days_since_last_irrigation(
                    field, target_date
                ),
            )
            ...
            schedule.append(
                {
                    ...
                    "log": (
                        IrrigationLogSerializer(log_on_day).data if log_on_day else None
                    ),
                }
            )
        ...
        summary = {
            ...
            "next_recommended_irrigation": next(
                (
                    s["date"]
                    for s in schedule
                    if s["recommendation"]["action"] == "irrigate"
                ),
                None,
            ),
        }
    ...
    def _get_weather_forecast(self, field):
        ...
        try:
            ...
            if lat is None or lng is None:
                logger.warning(
                    "Cannot compute centroid for field %d — missing polygon data",
                    field.id,
                )
    ...
    def _get_fallback_weather(self):
        ...
        for i in range(7):
            ...
            weather[i] = {
                ...
                "description": ["Clear", "Partly Cloudy", "Light Rain", "Cloudy"][
                    seed % 4
                ],
                "icon": "01d",
                "source": "fallback",
    ...
    def _days_since_last_irrigation(self, field, target_date):
        ...
        last_log = (
            IrrigationLog.objects.filter(field=field, date__lt=target_date)
            .order_by("-date")
            .first()
        )
    ...
    def _get_recommendation(
        self, day_weather, crop_type, was_irrigated, days_since_last_irrigation
    ):
        """
        ...
        return Response(
            {
                ...
                "sources": [
                    {"value": s.value, "label": s.label} for s in IrrigationSource
                ],
            }
        )

    def post(self, request):
        serializer = IrrigationLogSerializer(
        ...
        return Response(
            IrrigationLogSerializer(log).data, status=status.HTTP_201_CREATED
        )
