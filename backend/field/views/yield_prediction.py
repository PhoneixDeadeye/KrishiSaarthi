...
class YieldPredictionView(APIView):
        ...
        return Response(
            {
                ...
                "comparison": {
                    ...
                    "vs_regional": round(
                        (prediction["yield_per_hectare"] / regional_avg - 1) * 100
                    ),
                    "rating": self._get_yield_rating(
                        prediction["yield_per_hectare"], regional_avg
                    ),
                },
                "factors": prediction["factors"],
                "recommendations": self._get_recommendations(
                    ndvi_data, model, crop_type
                ),
            }
        )
    ...
    def _get_ndvi_data(self, field):
            ...
            if "error" not in ee_data:
                ...
                if ndvi_ts and len(ndvi_ts) >= 3:
                    ...
                    if time_series:
                        ...
                        recent_avg = sum(t["ndvi"] for t in time_series[-4:]) / min(
                            4, len(time_series)
                        )
                        older_avg = (
                            sum(t["ndvi"] for t in time_series[-8:-4])
                            / min(4, len(time_series[:-4]))
        ...
        recent_avg = (
            sum(t["ndvi"] for t in time_series[-4:]) / 4
            if len(time_series) >= 4
            else current
        )
        older_avg = (
            sum(t["ndvi"] for t in time_series[-8:-4]) / 4
            if len(time_series) >= 8
            else current
        )
        trend = (
