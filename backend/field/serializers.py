...
from .models import FieldData, FieldLog, FieldAlert, Pest, IrrigationLog, CropHealthScan

class CropHealthScanSerializer(serializers.ModelSerializer):
    class Meta:
        model = CropHealthScan
        fields = [
            "id", "field", "image_path", "detected_disease", 
            "confidence_score", "recommendation", "severity", "scanned_at"
        ]
        read_only_fields = ["id", "scanned_at", "detected_disease", "confidence_score", "recommendation", "severity"]
...
class FieldLogSerializer(serializers.ModelSerializer):
    field_id = serializers.PrimaryKeyRelatedField(
        queryset=FieldData.objects.none(),
        source="field",
        required=False,
        allow_null=True,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if request and hasattr(request, "user"):
            self.fields["field_id"].queryset = FieldData.objects.filter(
                user=request.user
            )
