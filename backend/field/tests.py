...
class AnalyticsAPITest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="testanalytics", password="password"
        )
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)
    ...
    def test_ee_analysis_view(self, mock_fetch):
        ...
        response = self.client.get(
            reverse("fieldAnalysis"), {"field_id": self.field.id}
        )
    ...
    def test_weather_view(self, mock_get):
        ...
        mock_resp_current.json.return_value = {
            "main": {"temp": 25},
            "weather": [{"main": "Clear"}],
        }
...
class SoilAdviceAPITest(TestCase):
    ...
    def test_soil_advice_ai_error_fallback(self, mock_model_class):
        ...
        response = self.client.post(
            reverse("soilAdvice"), {"N": 80, "P": 40, "K": 60, "pH": 6.5}, format="json"
        )
