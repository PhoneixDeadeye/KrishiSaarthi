    ...
    def ready(self):
        try:
            ...
            try:
                ...
                if not project_id:
                    logger.warning(
                        "GEE_PROJECT not set in .env, defaulting to 'nabard-field-data'"
                    )
                    project_id = "nabard-field-data"

                logger.info(
                    "Attempting to initialize Earth Engine with project: %s", project_id
                )
                ee.Initialize(project=project_id)
                logger.info(
                    "Earth Engine initialized successfully with project: %s", project_id
                )
            except ee.EEException as e:
                logger.error("Earth Engine initialization failed (EEException): %s", e)
                logger.error(
                    "Make sure you have run 'earthengine authenticate' and your GEE_PROJECT has Earthengine API enabled."
                )
            ...
            logger.warning(
                "earthengine-api not installed. Earth Engine features disabled."
            )
        except Exception as e:
