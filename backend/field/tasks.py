    ...
    for field in fields:
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    "Weather API returned %d for field %d",
            ...
            logger.error(
                "Network error updating weather for field %d: %s", field.id, exc
            )
            continue
...
def update_satellite_data(self):
    ...
    for field in fields:
        try:
            ...
            logger.error(
                "Error updating satellite data for field %d: %s", field.id, exc
            )
            continue
...
def generate_daily_reports():
    ...
    for user in users:
        try:
            ...
            for field in fields:
                last_log = (
                    FieldLog.objects.filter(user=user, field=field)
                    .order_by("-date")
                    .first()
                )
...
def cleanup_old_logs():
    ...
    deleted_count, _ = FieldLog.objects.filter(date__lt=cutoff_date).delete()
...
def generate_field_analytics(field_id, start_date=None, end_date=None):
    ...
    try:
        ...
        logs = FieldLog.objects.filter(field=field, date__range=[start_date, end_date])
