...
def detect_awd_from_ndwi(
        ...
        if val > wet_threshold:
            if state == "dry":
                ...
                if current_period_start:
                    dry_periods.append(
                        {"start": current_period_start, "end": date, "type": "dry"}
                    )
            if state != "wet":
            ...
            if state == "wet" and current_period_start:
                wet_periods.append(
                    {"start": current_period_start, "end": date, "type": "wet"}
                )
            if state != "dry":
                current_period_start = date
            state = "dry"
    ...
    return {
        ...
        "statistics": {
            ...
            "thresholds": {"wet": wet_threshold, "dry": dry_threshold},
        },
        "periods": {
        ...
        "benefits": {
            "water_savings": (
                f"~{round(base_water_savings_percent)}% reduction in irrigation water"
                if awd_detected
                else "Potential 15-30% savings with AWD"
            ),
            "emissions": (
                f"~{round(methane_reduction_percent)}% reduction in methane emissions"
                if awd_detected
                else "Potential 30-50% reduction with AWD"
            ),
