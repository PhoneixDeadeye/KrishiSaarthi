...
def load_risk_model(
    ...
    if not os.path.exists(model_path):
        logger.warning(
            "Risk model not found at %s. Risk prediction will return fallback values.",
            model_path,
        )
        return None, None
    ...
    if not os.path.exists(scaler_path):
        logger.warning(
            "Risk scaler not found at %s. Risk prediction will return fallback values.",
            scaler_path,
        )
        return None, None
...
def predict_risk_from_values(
    sequence: Union[List, Dict], device: str = DEVICE
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if prob > 0.7:
            ...
            risk_level = "Low"
            recommendation = (
                "Conditions appear favorable. Continue regular maintenance."
            )
