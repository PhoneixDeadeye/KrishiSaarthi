...
from .models import Season, CostEntry, Revenue, CostCategory, InsuranceClaim, FinanceTransaction
...
class FinanceTransactionSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'field', 'type', 'category', 'amount', 'date', 'notes', 'created_at'
        ]
    ...
    def validate_area_affected_acres(self, value):
        if value <= 0:
            raise serializers.ValidationError(
                "Area affected must be greater than zero."
            )
        return value
