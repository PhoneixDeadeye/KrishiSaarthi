...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options["force"]:
            ...
            self.stdout.write(
                self.style.WARNING(f"Deleted {deleted} existing schemes.")
            )

        if GovernmentScheme.objects.exists():
            self.stdout.write(
                self.style.SUCCESS("Schemes already seeded. Use --force to re-seed.")
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Successfully seeded {created} government schemes.")
        )
