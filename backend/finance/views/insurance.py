...
class InsuranceClaimView(APIView):
    ...
    def get(self, request):
        ...
        claims = InsuranceClaim.objects.filter(user=request.user).select_related(
            "field"
        )
            ...
            claims_data.append(
                {
                    ...
                    "claim_amount": (
                        float(claim.claim_amount) if claim.claim_amount else None
                    ),
                    "status": claim.status,
        ...
        approved_total = (
            claims.filter(status__in=["approved", "paid"]).aggregate(
